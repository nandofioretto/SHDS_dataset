...
public class Generator {
    ...
    public JSONObject generate() {
        ...
        try {
            ...
            try {
                FileWriter fileOut = new FileWriter("ruleCSV.txt", true);
    ...
    public JSONObject[] generateWithCSV(String fileName) {
        // All agents within a cluster share a constraint
        JSONObject[] jExperiments = new JSONObject[3];

        for(int i = 0; i < jExperiments.length; i++) {
            jExperiments[i] = new JSONObject();
        }

        try {
            for(int i = 0; i < jExperiments.length; i++) {
                jExperiments[i].put("horizon", timeHorizon);
                jExperiments[i].put("priceSchema", priceSchema);
            }

            //jExperiment.put("agents", );

            JSONObject[] jAgents = new JSONObject[3];
            for(int i = 0; i < jAgents.length; i++) jAgents[i] = new JSONObject();
            ...
            for (String agtName : topology.getAgents()) {
                ...
                jAgent.put("neighbors", jNeighbors);
                jAgent.put("backgroundLoad", generateBackgroundLoad());
                JSONArray[] jRules = ruleGenerator.generateSeparatedRules(nDevices, hType);
                for(int i = 0; i < jAgents.length; i++) {
                    jAgent.put("rules", jRules[i]);
                    jAgents[i].put(agtName, jAgent);
                }
            ...
            for(int i = 0; i < jExperiments.length; i++) {
                jExperiments[i].put("agents", jAgents[i]);
            }
